...
export default function App() {
  ...
  function reset() {
    setStep(0); setDataModel(null); setValidation(null);
    setSqlOutput(null); setErdData(null); setError('');
  }
  ...
  function handleGenerate(opts) {
    wrap(async function() {
      ...
      // Stamp db_type into data_model so it survives all subsequent API calls
  ...
  function handleAutoValidate() {
    wrap(async function() {
      // Always re-stamp db_type so sql_generator gets the right engine
  ...
  function handleFeedback(feedbackText) {
    wrap(async function() {
      var model = Object.assign({}, dataModel, { db_type: dbEngine });
      var res = await applyFeedbackAndGenerateSQL(model, feedbackText, operation);
  ...
  function handleGenerateERD(sql) {
    setErdLoading(true); setError('');
    generateERD(sql)
      .then(function(res) { setErdData(res); setStep(3); })
      .catch(function(e) { setError(e.message); })
      .finally(function() { setErdLoading(false); });
  }
